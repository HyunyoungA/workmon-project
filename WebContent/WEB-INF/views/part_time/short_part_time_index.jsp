...
<body>
	...
	<div id="notice_detail_wrap">
		<div id="job_id"
			...
			<div class="box">
				...
				<div id="part_time_writer_butt">
					...
					%>
					<button
						onclick='location.href="<%=request.getContextPath()%>/shotPartTimeWriter.rw"'>공고등록</button>
					<%
						}
					%>
				</div>
			...
			<table class="table1">
				...
				%>
				<tr>
					<td colspan="6">모집 중인 단기알바가 없습니다.</td>
				</tr>
				<%
					} else {
				%>
		...
		<div class="pagingArea" align="center">
			<!-- 맨 처음으로 -->
			<button
				onclick="location.href='<%=request.getContextPath()%>/shortPartTimeIndex.do?currentPage=1'"
				class="paging">&lt;&lt;</button>
			<!-- 이전 페이지로 -->
			<button
				onclick="location.href='<%=request.getContextPath()%>/shortPartTimeIndex.do?currentPage=<%=pi.getCurrentPage() - 1%>'"
			...
			%>
			<button id="numBtn"
				onclick="location.href='<%=request.getContextPath()%>/shortPartTimeIndex.do?currentPage=<%=p%>'"
			...
			<!-- 맨 마지막으로 -->
			<button
				onclick="location.href='<%=request.getContextPath()%>/shortPartTimeIndex.do?currentPage=<%=pi.getMaxPage()%>'"
				class="paging">&gt;&gt;</button>
	...
	<script>
		...
		/shortDetail.bo?pi.getCurrentPage()+1;
								} else {
